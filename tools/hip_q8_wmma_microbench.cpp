...
template <int TILES_N, int BM=16, int BN=16, int BK=16>
__global__ void q8_wmma_packed_multin_ahalf_global_xsplit_kernel(float* __restrict__ out,
                                                                 const half* __restrict__ bhalf,
    ...
    for (unsigned k0 = 0; k0 < K; k0 += BK) {
        if (wave < TILES_N) {
            rocwmma::load_matrix_sync(a, xhalf_hi + (uint64_t)m0 * K + k0, K);
            rocwmma::load_matrix_sync(b, bhalf + (uint64_t)k0 * N + n_wave, N);
            rocwmma::mma_sync(acc, a, b, acc);
            rocwmma::load_matrix_sync(a, xhalf_lo + (uint64_t)m0 * K + k0, K);
            rocwmma::mma_sync(acc, a, b, acc);
        }
    }
    if (wave < TILES_N) rocwmma::store_matrix_sync(out + (uint64_t)m0 * N + n_wave, acc, N, rocwmma::mem_row_major);
...
int main(int argc, char** argv) {
    ...
    float *dPackedMultiAhalfGlobalXSplit = nullptr, *dPackedMultiAhalfGlobalXSplit16 = nullptr;
    ...
    HIP_CHECK(hipMalloc(&dPackedMultiAhalfGlobalXSplit, (uint64_t)M * N * sizeof(float)));
    HIP_CHECK(hipMalloc(&dPackedMultiAhalfGlobalXSplit16, (uint64_t)M * N * sizeof(float)));
    ...
    HIP_CHECK(hipMemset(dPackedMultiAhalfGlobalXSplit, 0, (uint64_t)M * N * sizeof(float)));
    HIP_CHECK(hipMemset(dPackedMultiAhalfGlobalXSplit16, 0, (uint64_t)M * N * sizeof(float)));
    ...
    auto launch_packed_multi_ahalf_global_xsplit = [&]() {
        hipLaunchKernelGGL((q8_wmma_packed_multin_ahalf_global_xsplit_kernel<MULTI_N,16,16,16>), wmma_multi_grid, wmma_block, 0, 0,
                           dPackedMultiAhalfGlobalXSplit, dBhalf, dXhalfHi, dXhalfLo, M, K, N);
    };
    auto launch_packed_multi_ahalf_global_xsplit16 = [&]() {
        hipLaunchKernelGGL((q8_wmma_packed_multin_ahalf_global_xsplit_kernel<16,16,16,16>), wmma_multi16_grid, wmma_multi16_block, 0, 0,
                           dPackedMultiAhalfGlobalXSplit16, dBhalf, dXhalfHi, dXhalfLo, M, K, N);
    };
    ...
    launch_packed_multi_ahalf_global_xsplit(); HIP_CHECK(hipGetLastError());
    launch_packed_multi_ahalf_global_xsplit16(); HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipDeviceSynchronize());
    ...
    std::vector<float> hPackedMultiAhalfGlobalXSplit((uint64_t)M * N), hPackedMultiAhalfGlobalXSplit16((uint64_t)M * N);
    ...
    HIP_CHECK(hipMemcpy(hPackedMultiAhalfGlobalXSplit.data(), dPackedMultiAhalfGlobalXSplit, hPackedMultiAhalfGlobalXSplit.size() * sizeof(float), hipMemcpyDeviceToHost));
    HIP_CHECK(hipMemcpy(hPackedMultiAhalfGlobalXSplit16.data(), dPackedMultiAhalfGlobalXSplit16, hPackedMultiAhalfGlobalXSplit16.size() * sizeof(float), hipMemcpyDeviceToHost));
    ...
    compare("wmma_packed_multin_ahalf_global_xsplit8", hPackedMultiAhalfGlobalXSplit);
    compare("wmma_packed_multin_ahalf_global_xsplit16", hPackedMultiAhalfGlobalXSplit16);
    ...
    float packed_multi_ahalf_global_xsplit_ms = time_kernel(launch_packed_multi_ahalf_global_xsplit, iters);
    float packed_multi_ahalf_global_xsplit16_ms = time_kernel(launch_packed_multi_ahalf_global_xsplit16, iters);
    ...
    std::printf("wmma ahalf global N8:   %.4f ms  %.2f effective TFLOP/s  speedup %.3fx  incl_xrepack %.4f ms\n",
                packed_multi_ahalf_global_xsplit_ms, flops / (packed_multi_ahalf_global_xsplit_ms * 1.0e-3) / 1.0e12,
                cur_ms / packed_multi_ahalf_global_xsplit_ms, packed_multi_ahalf_global_xsplit_ms + x_repack_ms);
    std::printf("wmma ahalf global N16:  %.4f ms  %.2f effective TFLOP/s  speedup %.3fx  incl_xrepack %.4f ms\n",
                packed_multi_ahalf_global_xsplit16_ms, flops / (packed_multi_ahalf_global_xsplit16_ms * 1.0e-3) / 1.0e12,
                cur_ms / packed_multi_ahalf_global_xsplit16_ms, packed_multi_ahalf_global_xsplit16_ms + x_repack_ms);
    ...
    HIP_CHECK(hipFree(dPackedMultiAhalfGlobalXSplit));
    HIP_CHECK(hipFree(dPackedMultiAhalfGlobalXSplit16));
    return 0;
}
