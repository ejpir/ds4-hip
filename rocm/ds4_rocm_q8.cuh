...
__global__ static void shared_gate_up_swiglu_q8_0_w32_kernel(
        float *gate,
        float *up,
        float *mid,
        const unsigned char *wg,
...
__global__ static void grouped_q8_0_a_partial_w32_kernel(
        float *partial,
        const unsigned char *w,
        const float *heads,
        uint32_t n_groups,
...
__global__ static void q8_partial_sum_kernel(float *out, const float *partial, uint32_t out_dim, uint32_t n_splits) {
    const uint32_t row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row >= out_dim) return;
    float acc = 0.0f;
    for (uint32_t s = 0; s < n_splits; s++) acc += partial[(uint64_t)s * out_dim + row];
