...
__global__ static void moe_sum_slot_f16_kernel(float *out, const half *down_h, uint32_t out_dim, uint32_t n_expert, uint32_t n_tokens) {
    uint64_t gid = (uint64_t)blockIdx.x * blockDim.x + threadIdx.x;
    uint64_t n = (uint64_t)n_tokens * out_dim;
    if (gid >= n) return;
    uint32_t tok = gid / out_dim;
    ...
    for (uint32_t slot = 0; slot < n_expert; slot++) {
        acc += __half2float(down_h[((uint64_t)slot * n_tokens + tok) * out_dim + row]);
    }
    out[gid] = acc;
}
...
#pragma unroll
            for (uint32_t u = 0; u < KG; u++) {
                const uint32_t kk = kk0 + u;
                const float q = (float)((blk[16u + qbase + kk] >> shift) & 3u);
                shB[nn * (uint32_t)BK + kk] = __float2half(d * s * q - dm * m);
            }
        } else {
#pragma unroll
            for (uint32_t u = 0; u < KG; u++) {
                const uint32_t kk = kk0 + u;
                shB[nn * (uint32_t)BK + kk] = __float2half(0.0f);
            }
        }
    }
}
...
#pragma unroll
            for (uint32_t u = 0; u < KG; u++) {
                const uint32_t kk = kk0 + u;
                const float gq = (float)((gb[16u + qbase + kk] >> shift) & 3u);
                const float uq = (float)((ub[16u + qbase + kk] >> shift) & 3u);
                const uint32_t sj = nn * (uint32_t)BK + kk;
                shBg[sj] = __float2half(gd * gs * gq - gm * gmn);
                shBu[sj] = __float2half(ud * us * uq - um * umn);
            }
        } else {
#pragma unroll
            for (uint32_t u = 0; u < KG; u++) {
                const uint32_t kk = kk0 + u;
                const uint32_t sj = nn * (uint32_t)BK + kk;
                shBg[sj] = __float2half(0.0f);
                shBu[sj] = __float2half(0.0f);
            }
        }
    }
}
...
template <uint32_t PAIR_TILE, bool MID_F16=false, bool OUT_F16=false, bool DIRECT_OUT=false, bool SLOT_MAJOR=false>
...
__global__ static void moe_gate_up_mid_q2K_hotlist_wmma_kernel(
        float *mid_out,
        const char *gate_base,
        const char *up_base,
        const float *x,
    ...
    for (uint32_t j = tid; j < MTILES * BM * BN; j += blockDim.x) {
        ...
        const uint32_t bucket_row = m_group0 + mt * BM + mm;
        const uint32_t row = n0 + nn;
        if (bucket_row < count && row < expert_mid_dim) {
            const uint32_t pair = pairs[first + bucket_row];
            float g = shCg[j];
            float u = shCu[j];
            if (clamp > 1.0e-6f) {
            ...
            mid_out[(uint64_t)pair * expert_mid_dim + row] = moe_silu_oldhip(g) * u * weights[pair];
        }
    }
}
...
template <int MTILES=8, int BM=16, int BN=16, int BK=16, bool MID_F16=false, bool OUT_F16=false, bool DIRECT_OUT=false, bool SLOT_MAJOR=false>
...
template <int MTILES=8, int BM=16, int BN=16, int BK=16>
__global__ static void moe_gate_up_mid_q2K_hottile_wmma_kernel(
        float *mid_out,
        const char *gate_base,
        const char *up_base,
        const float *x,
