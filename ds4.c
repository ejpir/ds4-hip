...
static void layer_routed_moe_batch(
    ...
    for (uint32_t ai = 0; ai < n_active; ai++) {
        ...
        if (gate_in_dim != expert_in_dim || up_in_dim != expert_in_dim ||
            ...
            ds4_die("routed batch expert tensor layout mismatch");
        }
    }
...
#if defined(DS4_USE_GPU_API)
    const bool shared_down_f16_enabled = (getenv("DS4_CUDA_SHARED_DOWN_F16_OUT") != NULL ||
