...
static bool worker_progress_cb(void *ud, const char *event, int current, int total) {
    (void)total;
    agent_worker *w = ud;
    if (!w) return true;
    if (event && !strcmp(event, "prefill_chunk")) {
        pthread_mutex_lock(&w->mu);
...
static bool agent_kv_save_path(agent_worker *w, const char *path,
    ...
    ds4_kvstore_fill_header(h, 0, (uint8_t)quant_bits,
...
static void build_status_text(const agent_status *st, char *buf, size_t len) {
    ...
    case AGENT_WORKER_DRAINING:
        snprintf(buf, len, "ctx %s/%s | stopping after distributed cluster drains",
                 used, total_ctx);
        break;
