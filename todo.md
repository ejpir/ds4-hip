...
- Added GGUF-backed Q2_K MoE microbench/replay helpers:
  ...
  - Integrated the hot-expert-list Q2_K WMMA path behind `DS4_HIP_MOE_WMMA_HOT=1` (`DS4_HIP_MOE_WMMA_GATE_HOT`, default 128; `DS4_HIP_MOE_WMMA_DOWN_HOT`, default 64; optional `DS4_HIP_MOE_WMMA_LAYERS` layer/range filter). It is not default-on. Long-prompt first-token smoke: down-only `GATE_HOT=99999 DOWN_HOT=32/64` ≈55.3 t/s, first token `We`; aggressive full-layer `GATE_HOT=64 DOWN_HOT=32` ≈58.65 t/s, first token `We`, prompt-graph top matched on a short prompt. Stage profile layer32/pos2048 routed_moe improved from ≈363ms current to ≈298ms with gate64/down32 under profiling. Follow-up 32-token greedy-logprob checks against the scalar/shared-X MoE path found full-layer drift on the 5707-token prompt: gate64/down32 and down-only thresholds drift at token 10 (` prompt` -> ` text`), gate128 variants drift at token 19 (` core` -> ` key`), and even very high down thresholds still drift. A diagnostic real-layer filter, `DS4_HIP_MOE_WMMA_LAYERS=14-42` with gate64/down32, matched the 32-token scalar continuation on the same prompt and measured ~7% relative prefill gain in a throttled A/B, but it remains experimental and needs broader exact validation. Treat Q2_K WMMA as first-token/perf experimental only until a precision-neutral formulation is found.
