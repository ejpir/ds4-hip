...
- Added an opt-in ROCm/CUDA-port shared-expert batch gate/up fusion: `DS4_CUDA_SHARED_GATE_UP_BATCH_FUSED=1` computes Q8 gate+up and SwiGLU in one shared-X tiled prefill kernel, with tuning knobs `DS4_CUDA_SHARED_GATE_UP_BATCH_TILE` and `DS4_CUDA_SHARED_GATE_UP_BATCH_SHARED_X_BLOCKS`. On the packed-attention/MoE-WMMA profile it reduced profiled `shared_gate_up` from roughly `~1.2s` to `~0.94-0.99s`, but total prefill remains noisy (`~92 tok/s` best zero-copy repeats similar to unfused high runs). Forced-stream quality gate `/tmp/ds4_rocm_quality_shared_gateup_fused` passes; keep this as an opt-in micro-optimization rather than promoting it into the main fast recipe.
