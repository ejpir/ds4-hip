...
- Added Q2_K expert-batch substage profiling under `DS4_CUDA_MOE_PROFILE=1`. On `/tmp/prompt1741.txt --ctx 4096 -n 1` with full-copy + MoE WMMA hot + attention-output hipBLAS, the 43-layer MoE total was `~8.92s`: bucket `~0.007s`, gate scalar `~1.12s`, gate WMMA `~4.33s`, down scalar `~0.48s`, down WMMA `~2.93s`, sum `~0.05s`. This confirms the remaining MoE bottleneck is the WMMA hot kernels themselves, especially gate/up. Diagnostic attempts to compact hot WMMA scheduling (`DS4_CUDA_MOE_WMMA_SPLIT_HOT=1`, `DS4_CUDA_MOE_WMMA_TILE_HOT=1`) and to change WMMA M tiles (`DS4_CUDA_MOE_WMMA_MTILES=4|16`) were slower; keep them opt-in/rejected.
